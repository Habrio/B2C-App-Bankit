...
import { ArrowLeftIcon, SearchIcon, EyeOffIcon, ChevronRightIcon, ChatIcon, BankIcon, CardsIcon } from '../constants/icons';
...
const BankAccountsView: React.FC<{ onNavigate: (screen: Screen) => void }> = ({ onNavigate }) => {
        ...
        <div className="px-4 pb-4 space-y-5">
            ...
            <div className="bg-white p-4 rounded-2xl shadow-sm flex items-center">
                <div className="flex-grow">
                    <p className="font-semibold text-gray-800 text-sm">Need Cash? Get Instant</p>
                    <p className="font-bold text-base text-secondary">Loan upto ₹5 Lakh!</p>
                    ...
                    {recentTransactions.map(tx => (
                        <div key={tx.id} className="flex items-center p-3 border-b border-gray-100 last:border-b-0">
                            ...
                            <div className="text-right">
                                <p className="font-bold text-sm text-red-500">₹{tx.amount} <span className="text-base">↗</span></p>
...
const ConnectedBankingScreen: React.FC<ConnectedBankingScreenProps> = ({ onBack, onNavigate }) => {
    ...
    const tabs = [
        { name: 'Accounts', icon: BankIcon, content: <BankAccountsView onNavigate={onNavigate} /> },
        { name: 'Cards', icon: CardsIcon, content: <CardsScreen onNavigate={onNavigate} /> }
    ];
