...
import { ArrowLeftIcon, SearchIcon, XIcon, ChevronRightIcon, InfoIcon, CreditCardIcon } from '../constants/icons';

type JourneyStep = 'LIST' | 'PURCHASE' | 'PAYMENT' | 'SUCCESS';

interface GiftCardJourneyProps {
  onClose: () => void;
  onPurchaseSuccess: (card: PurchasedGiftCard) => void;
  initialBrand?: GiftCardBrand;
}

const GiftCardJourney: React.FC<GiftCardJourneyProps> = ({ onClose, onPurchaseSuccess, initialBrand }) => {
  ...
  const renderContent = () => {
    if (step === 'PAYMENT') {
        return (
            <div className="bg-gray-800 rounded-t-2xl p-4 text-white">
                <div className="flex justify-between items-center mb-4">
                    <h3 className="font-bold text-lg">FindiBankit Pay</h3>
                ...
                <div className="my-6">
                    <p className="font-semibold mb-2">RECOMMENDED METHODS</p>
                    <div className="bg-gray-900 rounded-lg p-4 flex justify-between items-center">
                        ...
                        <div className="w-5 h-5 rounded-full border-2 border-primary bg-primary ring-2 ring-primary/50"></div>
                    </div>
                </div>

                <div className="absolute bottom-0 left-0 right-0 p-4 bg-gray-900 border-t border-gray-700 flex justify-between items-center">
                    <div>
                        <p className="text-sm text-gray-400 line-through">₹{Number(customAmount) || selectedAmount}</p>
                        <p className="text-xl font-bold">₹{finalPrice.toFixed(2)}</p>
                    </div>
                    <button onClick={handlePayment} className="bg-white text-black font-bold py-3 px-6 rounded-lg flex items-center space-x-2">
                        <span>Pay now</span>
        ...
        return (
            <div className="bg-gray-900 rounded-t-2xl p-4 text-white relative">
                 <div className="flex justify-between items-center mb-4">
                    ...
                    <h3 className="font-bold">{selectedBrand.name} gift card</h3>
                ...
                <div className="w-full aspect-video rounded-xl overflow-hidden bg-cover bg-center mb-4" style={{backgroundImage: `url(${selectedBrand.cardImage})`}}>
                </div>
                ...
                <input
                    ...
                    className="w-full bg-gray-800 border border-gray-700 rounded-lg p-3 mb-4 focus:outline-none focus:ring-2 focus:ring-primary"
                />

                <div className="bg-green-900/50 text-green-300 font-semibold p-2 rounded-lg text-center mb-24">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4 bg-gray-800 border-t border-gray-700 flex justify-between items-center">
                    ...
                    <button onClick={handleProceedToPayment} className="bg-white text-black font-bold py-3 px-6 rounded-lg">Buy Now</button>
                </div>
            </div>
        )
    }
        ...
        <header className="p-4 flex-shrink-0">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-gray-800 rounded-full py-2 pl-10 pr-4 focus:outline-none focus:ring-2 focus:ring-primary"
            />
          </div>
        </header>
  ...
  return (
    <div className="absolute inset-0 bg-black bg-opacity-70 z-50 flex flex-col justify-end">
        <div className={`w-full ${step === 'LIST' ? 'h-full' : ''} ${step === 'PURCHASE' ? 'h-[95%]' : ''} ${step === 'PAYMENT' ? 'h-[70%]' : ''} bg-gray-900 transition-all duration-300 rounded-t-2xl`}>
            {renderContent()}
        </div>
    </div>
  );
