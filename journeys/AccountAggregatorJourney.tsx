...
const ConnectedBankingJourney: React.FC<ConnectedBankingJourneyProps> = ({ onClose, onSuccess }) => {
    ...
    useEffect(() => {
        if (step === 'DISCOVERY') {
            ...
            setTimeout(() => {
                setDiscoveredAccounts([
                    { id: '1', name: 'Federal Bank', type: 'Savings Account', number: 'XX9147', logo: 'https://logo.clearbit.com/federalbank.co.in', selected: true },
                    { id: '2', name: 'Punjab National Bank', type: 'Savings Account', number: 'XX1019', logo: 'https://logo.clearbit.com/pnbindia.in', selected: true },
                    { id: '3', name: 'HDFC Bank Credit Card', type: 'Credit Card', number: 'XX7212', logo: 'https://cdn.freelogovectors.net/wp-content/uploads/2023/09/hdfc-bank-logo-freelogovectors.net_.png', selected: true },
                ]);
            }, 1500);
            setTimeout(() => {
    ...
    const handleApproval = () => {
        setSelectedAccounts(discoveredAccounts.filter(acc => acc.selected));
        setStep('PROCESSING');
    ...
    const LandingScreen = () => {
        const benefits = [
            "One view of all your bank accounts & credit cards for you to stay on top of your money",
            ...
            <div className="bg-gray-100 text-gray-800 h-full flex flex-col">
                ...
                <main className="flex-grow overflow-y-auto pb-24 px-4 space-y-6">
                    ...
                    <div className="bg-white p-5 rounded-2xl shadow-sm">
                        <h2 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-4">Benefits</h2>
                    ...
                    {/* How We Do It Card */}
                    <div className="bg-white p-5 rounded-2xl shadow-sm">
                        <h2 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-4">How we do it?</h2>
                        <p className="text-gray-700 mb-6 text-sm">RBI's Account Aggregator tech makes sharing your account info simple and secure.</p>
                     ...
                     {/* FAQs Card */}
                    <div className="bg-white p-5 rounded-2xl shadow-sm">
                         <h2 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">FAQs</h2>
                         ...
                         <button className="font-semibold text-secondary mt-4 text-sm">See All &gt;</button>
                    </div>
                </main>
    ...
    const MobileInputScreen = () => (
        <div className="absolute inset-0 bg-black/60 flex flex-col justify-end" onClick={() => setStep('LANDING')}>
            <div className="bg-white rounded-t-2xl p-6 pt-4" onClick={e => e.stopPropagation()}>
                ...
                <h2 className="text-xl font-bold text-gray-900">Enter mobile number linked to your account</h2>
                <p className="text-gray-500 mt-1 mb-6 text-sm">Bank accounts only linked to this number can be tracked</p>
                <div className="flex items-center space-x-2">
                    <div className="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center">🇮🇳</div>
                    <p className="text-lg font-semibold tracking-wider">{mobileNumber}</p>
                </div>
                ...
                <main className="flex-grow flex flex-col items-center justify-center text-center -mt-12">
                    <div className="w-full max-w-sm">
                        <div className="flex items-center justify-center space-x-2 mb-4">
                            ...
                            <span className="text-xl font-bold text-gray-800">FindiBankit</span>
                        </div>
                        <h1 className="text-xl font-bold text-gray-900">Enter OTP to fetch your accounts</h1>
                        <p className="text-gray-500 mt-1 text-sm">
                            Sent to +91 {mobileNumber}
                        </p>
                        <div className="flex justify-center space-x-3 my-8 text-3xl text-gray-400">
                            {Array.from({length: 6}).map((_, i) => <span key={i}>X</span>)}
                        </div>
                         <p className="text-gray-500 text-sm">Resend OTP in 00:30</p>
                    </div>
                </main>
    ...
    const DiscoveryScreen = () => (
        <div className="flex flex-col h-full bg-gray-100 p-6">
            ...
            <main className="flex-grow flex flex-col -mt-8">
                <h1 className="text-xl font-bold text-gray-900">We found {discoveredAccounts.length} accounts.</h1>
                <h2 className="text-xl font-bold text-gray-500">Choose which to connect.</h2>
                {isDiscovering && <p className="text-gray-500 mt-1 text-sm">Discovering more accounts</p>}
                ...
                <div className="bg-white rounded-2xl shadow-sm p-4 my-6 space-y-4">
                    {discoveredAccounts.map(acc => (
                         <div key={acc.id} onClick={() => toggleAccountSelection(acc.id)} className="flex items-center">
                            <img src={acc.logo} alt={acc.name} className="w-10 h-10 rounded-lg mr-4" />
                            <div className="flex-grow">
                                <p className="font-bold text-sm">{acc.name}</p>
                                <p className="text-sm text-gray-600">{acc.type} • {acc.number}</p>
                            </div>
    ...
    const ProcessingScreen = () => {
        ...
        return (
            <div className="flex flex-col h-full bg-white p-6 justify-center">
                         ...
                         <React.Fragment key={acc.id}>
                            <div className="flex justify-between items-center">
                                <div className="flex items-center">
                                    <img src={acc.logo} className="w-10 h-10 rounded-lg" alt={acc.name} />
                                    <div className="ml-2">
                                        <p className="font-bold text-sm">{acc.name}</p>
                            ...
                            {index < selectedAccounts.length - 1 && <div className="h-px bg-gray-200 my-2" />}
                        </React.Fragment>
                    ))}
                ...
                <p className="text-center text-gray-600 mt-4 text-sm">Getting your dashboard ready for review...</p>
            </div>
        )
    };
